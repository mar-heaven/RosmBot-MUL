...
type Chat struct {
	...
	// MessageContent holds the value of the "message_content" field.
...
func (*Chat) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case chat.FieldID, chat.FieldGroupID, chat.FieldUserID, chat.FieldTime:
			values[i] = new(sql.NullInt64)
		case chat.FieldMessageType, chat.FieldMessageContent:
	...
	for i := range columns {
		...
		case chat.FieldMessageType:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				c.MessageType = value.String
			}
		case chat.FieldMessageContent:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field message_content", values[i])
			} else if value.Valid {
				c.MessageContent = value.String
			}
		default:
...
func (c *Chat) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("message_content=")
	builder.WriteString(c.MessageContent)
	builder.WriteByte(')')
